...
from arduino_board_collection.boards.sensor_boards.basic.hx711_board.board import HX711Board, HX711Module
...
class TensileTesterApi(BoardApi):
    ...
    def run_test(self, offset, scale, maximum_force, maximum_strain, specimen_length, maximum_speed=DEFAULT_MAX_SPEED, move_back=True,
                 min_force=1, on_finish=None, minimum_find_wobble_count=DEFAULT_WOBBLE_COUNT):
    ...
    def move_to(self, mm, allowed_error=DEFAULT_ERROR):
        ...
        if self.moving:
            self.moving = False
            time.sleep(2 + self.move_sleep_time)
        self.moving = True
        mm = float(mm)
        board: TensileTestBoard = self.linked_boards[0]
