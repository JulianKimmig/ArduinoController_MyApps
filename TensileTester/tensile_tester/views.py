    ...
    def post(self,request):
        ...
        if form.is_valid():
            ...
            def _result(time_data,stress_strain_data):
                ...
                regname = ''.join(c if c in '-_()abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789' else "_" for c in tensile_test.name)
                plt.figure()
            ...
            tensilertesterapi.run_test(maximum_force=tensile_test.maximum_force,offset=tensile_test.offset,scale=tensile_test.scale,maximum_strain=tensile_test.maximum_strain,specimen_length=tensile_test.specimen_length,maximum_speed=tensile_test.maximum_speed,on_finish=_result)
            time.sleep(0.1)
            return redirect('tensile_tester:running_measurement')
        return render(request, "tensile_tester_measurement.html",{'form':form})
...
def view_test(request,id):
    ...
    return render(request, "tensile_tester_view_test.html",dict(test=tensile_test,
                                                                data = mark_safe(json.dumps(dict(time=data['time'].tolist(),
